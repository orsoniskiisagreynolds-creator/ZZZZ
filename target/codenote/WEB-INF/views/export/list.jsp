...
<!DOCTYPE html>
<html><head>
    <meta charset="UTF-8"><title>导出记录列表</title>
...
<nav class="navbar navbar-dark mb-4" style="background:rgba(0,0,0,0.3)">
    ...
    <a href="${pageContext.request.contextPath}/export/create" class="btn btn-primary btn-sm">新建导出记录</a>
</nav>
<div class="container"><h2 class="mb-4">导出记录列表</h2>
<div class="row">
<c:forEach var="item" items="${list}">
<div class="col-md-4 mb-3"><div class="card h-100"><div class="card-body">
<h5 class="card-title" style="color:#4fc3f7">${item.fileName}</h5>
</div><div class="card-footer bg-transparent">
<a href="${pageContext.request.contextPath}/export/edit/${item.id}" class="btn btn-sm btn-outline-info">编辑</a>
<a href="${pageContext.request.contextPath}/export/delete/${item.id}" class="btn btn-sm btn-outline-danger" onclick="return confirm('确定删除?')">删除</a>
</div></div></div>
</c:forEach>
</div></div>
</body></html>