...
<!DOCTYPE html>
<html><head>
    <meta charset="UTF-8"><title>外部资源列表</title>
...
<nav class="navbar navbar-dark mb-4" style="background:rgba(0,0,0,0.3)">
    ...
    <a href="${pageContext.request.contextPath}/resource/create" class="btn btn-primary btn-sm">新建外部资源</a>
</nav>
<div class="container"><h2 class="mb-4">外部资源列表</h2>
<div class="row">
...
<a href="${pageContext.request.contextPath}/resource/edit/${item.id}" class="btn btn-sm btn-outline-info">编辑</a>
<a href="${pageContext.request.contextPath}/resource/delete/${item.id}" class="btn btn-sm btn-outline-danger" onclick="return confirm('确定删除?')">删除</a>
</div></div></div>
</c:forEach>
</div></div>
</body></html>